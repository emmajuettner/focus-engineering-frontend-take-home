...
const resolvers = {
    State: {
        commuteTimes: async (parent: State, args: { year?: string }) => {
            ...
            if (!results) {
                const response = await fetch(
                    `https://honolulu-api.datausa.io/tesseract/data.jsonrecords?cube=acs_ygt_travel_time_to_work_5&measures=Commuter+Population,Commuter+Population+Moe&include=State:${id}&drilldowns=State,Year,Travel+Time`
                )
        ...
        commuteMethods: async (parent: State, args: { year?: string }) => {
            ...
            if (!results) {
                const response = await fetch(
                    `https://honolulu-api.datausa.io/tesseract/data.jsonrecords?cube=acs_ygt_means_of_transportation_to_work_5&measures=Commute+Means,Commute+Means+Moe&include=State:${id}&drilldowns=Group`
                )
                const data = await response.json()
        ...
        collegeConcentrations: async (
            ...
            if (!results) {
                const response = await fetch(
                    `https://honolulu-api.datausa.io/tesseract/data.jsonrecords?cube=ipeds_completions&drilldowns=CIP6,Year,CIP4&include=State:${id}&locale=en&measures=Completions`
                )
                const data = await response.json()
...
const logout = (req: Request, res: Response, next: NextFunction) => {
    // @ts-ignore passport 0.7 requires callback at runtime despite types
    ...
    app.use(
        ...
        expressMiddleware(server, {
            context: async ({ req }) => ({
                ...
                getUser: () => req.user,
                // @ts-ignore passport 0.7 requires callback at runtime
                logout: () => req.logout(() => {}),
            }),
        })
    )
